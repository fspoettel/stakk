...
import getAlbum from '@stakk/services/api/spotify/getAlbum';
import getImage from '@stakk/lib/api/getImage';
import { ValidationError } from '@stakk/lib/api/errors';
import { toImageUrl, toStackItem } from '@stakk/lib/api/formatSpotifyAlbum';
import wrapErrorHandler from '@stakk/lib/api/wrapErrorHandler';

async function postSpotifyAlbum(req: NextApiRequest, res: NextApiResponse) {
  const { url } = JSON.parse(req.body);
  if (!url) throw new ValidationError('request missing required attribute `url`');

  const debug = _debug('stakk:spotify_album');

  debug('retrieving access token...');
  const { accessToken } = await getAccessToken();

  debug('retrieving album...');
  const playlist = await getAlbum(accessToken, url);
...
export default wrapErrorHandler(postSpotifyAlbum);
